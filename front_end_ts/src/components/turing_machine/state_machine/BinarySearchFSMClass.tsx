...
export class BinarySearchFSMClass {
    ...
    calcStep(): boolean {
        ...
        // 循环部分
        if (this.step_ === 5 || this.state_ === States.COMPARE || this.state_ === States.WRITE_LOW || this.state_ === States.WRITE_HIGH) {
        ...
        return false; // 默认返回 false 防止没有返回值的情况
    }
    ...
    calcHistory(): { [key: number]: FSMHistoryEntry } {
        ...
        history[this.step_] = {
            step: this.step_,
            state: this.state_,
            inputTape: this.inputTape_,
            workTape: this.workTape_,
            resultTape: this.resultTape_
        };
